  ...
  try {
    ...
    if (avatar && typeof avatar === "object" && avatar.size > 0) {
      try {
        ...
        if (avatarUrl) {
          basePayload.avatar_url = avatarUrl;
        }
      } catch (error) {
        avatarWarning = error.message || "Avatar upload failed.";
        console.error("[AVATAR_UPLOAD_WARNING]", avatarWarning);
      }
    }

    const updatedProfile = await updateProfileByCharacterId(profileCode, basePayload);
